...
import com.application.jetbill.model.entity.CollectionBookPK;
...
public interface CollectionBookRepository extends JpaRepository<CollectionBook, CollectionBookPK> {
    ...
    void addBookToCollection(@Param("bookId") Integer bookId,
                             @Param("collectionId") Integer collectionId,
                             @Param("addedDate") LocalDateTime addedDate);

    @Modifying
    @Transactional
    @Query(value = "DELETE FROM collection_books WHERE book_id = :bookId " +
            "AND collection_id = :collectionId", nativeQuery = true)
    void deleteByBookAndCollection(@Param("bookId") Integer bookId,
                                   @Param("collectionId") Integer collectionId);
